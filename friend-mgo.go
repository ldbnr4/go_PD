...
func (ctrl *Controller) DeclineReqMgo(fuidStr string) {
	mgoRmFrmSet(ctrl.userCol, ctrl.ServerUser.ObjectId, "friendReqs", bson.ObjectIdHex(fuidStr))
}

func (ctrl *Controller) SendReqMgo(fuidStr string) {
	mgoAddToSet(ctrl.userCol, bson.ObjectIdHex(fuidStr), "friendReqs", ctrl.ServerUser.ObjectId)
}
